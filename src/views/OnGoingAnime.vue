        ...
        <div class="col-12">
          <div class="row">
            <div class="col-12">
              ...
              <p class="section__text">Daftar daftar anime yang masih berlangung saat ini.</p>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    getOnGoingAnime() {
      this.$otaku.get(`/ongoing-anime/page/${this.nextPage}`)
        .then(res => {
    ...
    calculatePagination(res) {
      ...
      pagination.map((val, index) => {
        newPagination.push(val.replace('https:/otakudesu.watch/ongoing-anime/page/', '').replace('/', ''));
      });
