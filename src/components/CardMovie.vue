<template>
  <div v-for="(anime, index) in anime_list" :key="index" class="col-6 col-sm-4 col-lg-3 col-xl-2">
    <div class="card">
      <router-link :to="{
        name: 'wacth-anime',
      ...
      <span class="card__rating"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        ...
        </svg> {{ anime.score ? anime.score : '?' }}</span>
      <h3 class="card__title">
        <router-link :to="{
          name: 'wacth-anime',
          params: { slug: anime.id.replace('/', '') },
        }">{{ anime.title ? anime.title : anime.anime_name }}</router-link>
      </h3>
      <ul class="card__list">
        <li>{{ anime.episode }}</li>
        <li v-if="anime.day_updated">{{ anime.day_updated }} - {{ anime.uploaded_on }}</li>
        <li v-else>{{ anime.uploaded_on ? anime.uploaded_on : anime.release_date }}</li>
      </ul>
    </div>
  </div>
</template>
