<template>
  ...
  <ul class="header__nav">
    <li class="header__nav-item">
      <router-link class="header__nav-link" :to="{ name: 'home' }">Beranda</router-link>
    </li>
    <li class="header__nav-item">
      <router-link class="header__nav-link" :to="{ name: 'ongoing-anime' }">Berlansung</router-link>
    </li>
    <li class="header__nav-item">
      <router-link class="header__nav-link" :to="{ name: 'complete-anime' }">Tamat</router-link>
    </li>
    <li class="header__nav-item">
      <router-link class="header__nav-link" :to="{ name: 'schedule-anime' }">Jadwal</router-link>
    </li>
    <li class="header__nav-item">
      <a class="header__nav-link header__nav-link--more" href="#" role="button" id="dropdownMenu3"
      ...
      <ul class="dropdown-menu header__nav-menu header__nav-menu--scroll" aria-labelledby="dropdownMenu3">
        <li>
          <router-link class="header__nav-link" :to="{ name: 'genre-list' }">Genre</router-link>
        </li>
        <li>
          <a href="https://otakudesu.watch/">Otakudesu</a>
        </li>
        <li>
          <router-link class="header__nav-link" :to="{ name: 'contact' }">Kontak</router-link>
        </li>
        <li>
          <router-link class="header__nav-link" :to="{ name: 'about-us' }">Tentang kami</router-link>
        </li>
        <!-- @guest -->
        <li>
          <router-link class="header__nav-link" :to="{ name: 'login' }">Masuk</router-link>
        </li>
        <li>
          <router-link class="header__nav-link" :to="{ name: 'register' }">Daftar</router-link>
        </li>
        <li>
          <router-link class="header__nav-link" :to="{ name: 'forgot-password' }">Lupa katasandi</router-link>
        </li>
        <!-- @else
        <li><a href="#">{{ __('pages.btn_profile') }}</a></li>
        @endguest -->
      </ul>
    </li>
  </ul>
  ...
  <div class="header__actions">
    <form class="header__form" @submit.prevent="toSearch">
      <input name="search" class="header__form-input" type="text" placeholder="Cari disini..." v-model="keyword">
      <button class="header__form-btn" type="submit"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          <path
            d="M21.71,20.29,18,16.61A9,9,0,1,0,16.61,18l3.68,3.68a1,1,0,0,0,1.42,0A1,1,0,0,0,21.71,20.29ZM11,18a7,7,0,1,1,7-7A7,7,0,0,1,11,18Z" />
        </svg></button>
      <button type="submit" class="header__form-close"><svg width="20" height="20" viewBox="0 0 20 20" fill="none"
    ...
    <!-- @guest -->
    <!-- <router-link class="header__user" :to="{ name: 'login' }">
      <span>Masuk</span>
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        <path
    ...
    <!-- @else -->
    <a href="#" @click.prevent="logout" class="header__user">
      <span>Keluar</span>
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        <path
          d="M20,12a1,1,0,0,0-1-1H11.41l2.3-2.29a1,1,0,1,0-1.42-1.42l-4,4a1,1,0,0,0-.21.33,1,1,0,0,0,0,.76,1,1,0,0,0,.21.33l4,4a1,1,0,0,0,1.42,0,1,1,0,0,0,0-1.42L11.41,13H19A1,1,0,0,0,20,12ZM17,2H7A3,3,0,0,0,4,5V19a3,3,0,0,0,3,3H17a3,3,0,0,0,3-3V16a1,1,0,0,0-2,0v3a1,1,0,0,1-1,1H7a1,1,0,0,1-1-1V5A1,1,0,0,1,7,4H17a1,1,0,0,1,1,1V8a1,1,0,0,0,2,0V5A3,3,0,0,0,17,2Z" />
...
<script>
export default {
  name: 'HeaderAtomComponent',
  data() {
    return {
      keyword: "",
    }
  },
  emits: ['getSearchKeyword'],
  methods: {
    toSearch() {
      if (this.keyword == "") {
        return false;
